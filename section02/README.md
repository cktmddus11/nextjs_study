...
├── index.tsx           // "/" 라우트
├── about.tsx           // "/about" 라우트
└── blog/[id].tsx       // 동적 라우트 "/blog/:id"
└── blog/[...id].tsx    // 동적 라우트 Catch All Segement "/blog/id/id/id..."
└── blog/[[...id]].tsx  // 동적 라우트 Optional Catch All Segement "/blog", "/blog/id/id/id..."
```
### 2. 다양한 방식의 사전 렌더링 제공
느린 FCP 개선을 위한 다양한 렌더링 제공.
* 1) 서버 사이드 렌더링(SSR) 
  * 요청이 들어올 때 마다 사전 렌더링을 진행함.
  * 단점 : 사전 렌더링 지연시 JS 실행하여 서버 요청 시 서버지연에 화면 렌더링 영향받음.
  * 사용 : 최신 데이터 화면
* 2) 정적 사이트 생성(SSG)
  * 빌드 타임에 미리 페이지를 사전 렌더링 해둠.
  * 장점 : 사전 렌더링에 많은 시간이 소요되는 페이지더라도 사용자의 요청에는 매우 빠른 속도로 응답 가능.
  * 단점 : 매번 똑같은 페이지만 응답함, 최신 데이터 반영은 어렵다.
  * 사용 : 정적인 화면
* 3) 증분 정적 재생성(ISR)
  * SSG 페이지 일정 시간마다 재생성
  * 사용 : 시간 또는 요청 시(revalidate) 동적인 화면

## Page Router 의 단점
1. 페이지별 레이아웃 설정이 번거롭다.
2. 데이터 패칭이 페이지 컴포넌트에 집중된다.    
=> 데이터 페칭 후 page컴포넌트에 props로 전달해주어야하고 하위 컴포넌트가 더 존재한다면 계속 props로 전달해주어야함.
3. 불 필요한 컴포넌트들도 JS Bundle에 포함된다.   
=> 상호작용이 없는 컴포넌트들까지 포함됨.      
  상호작용(인터렉션) (사용자의 입력을 State로 보관해야함. Enter 키 입력이 발생하면 페이지를 이동시켜야함.